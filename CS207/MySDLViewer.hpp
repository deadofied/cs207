...
class SDLViewer {
 ...
 public:
  ...
  void draw_graph_nodes(const G& g) {
    ...
    { safe_lock mutex(this);
      ...
      for (typename G::size_type i = 0; i < g.num_nodes(); ++i) {
        ...
        coords_.push_back(p);
        sizes_.push_back(g.node(i).value());
        center += p;
      }
  ...
  void add_nodes(InputIterator first, InputIterator last,
                 Map& node_map) {
    return add_nodes(first, last, DefaultColor(), DefaultPosition(),DefaultSize(), node_map);
  }
  ...
  void add_nodes(InputIterator first, InputIterator last,
    ...
    return add_nodes(first, last, color_function, DefaultPosition(), DefaultSize(), node_map);
  }
  ...
  template <typename InputIterator,typename ColorFn, typename PointFn, typename SizeFn, typename Map>
  void add_nodes(InputIterator first, InputIterator last,ColorFn color_function, PointFn position_function, SizeFn size_function,Map& node_map) {
    // Lock for data update
    { safe_lock mutex(this);
      ...
      for (; first != last; ++first) {
        ...
        if (r.second) {   // new node was inserted
          ...
          sizes_.push_back(size_function(n));
  ...
  template <typename InputIterator, typename WidthFn, typename Map>
  void add_edges(InputIterator first, InputIterator last, WidthFn width_function, const Map& node_map) {
    // Lock for data update
    { safe_lock mutex(this);
      ...
      for (; first != last; ++first) {
        ...
        if (n1 != node_map.end() && n2 != node_map.end()) {
          ...
          widths_.push_back(width_function(edge));
          widths_.push_back(width_function(edge));
        }
      }
    }
 ...
 private:
  ...
  void init() {
    ...
    // Background Color
    glClearColor(255.0, 255.0, 204.0, 1.0);
    // Initialize View
  ...
  /**New render function with dynamic node size and dynamic edge size
  */
  void render2() {
    safe_lock mutex(this);
    ...
    for(unsigned i=0;i<coords_.size();++i){
      glPointSize(sizes_[i]);
      glBegin(GL_POINTS);
      glVertex3f(coords_[i].x,coords_[i].y,coords_[i].z);
    ...
    // Draw the lines
    glLineWidth(1);
    //glDrawElements(GL_LINES, edges_.size(),gltype<unsigned>::value, edges_.data());
    for(unsigned i=0;i<edges_.size();i=i+2){
      glLineWidth(widths_[i]);
      glBegin(GL_LINES);
      glVertex3f(coords_[edges_[i]].x,coords_[edges_[i]].y,coords_[edges_[i]].z);
      glVertex3f(coords_[edges_[i+1]].x,coords_[edges_[i+1]].y,coords_[edges_[i+1]].z);
